...
def main():
    ...
    with col3:
        ...
        show_pai_options = st.selectbox('Is there Periapical RL changes', ['None', 'Yes', 'No'])

        # Initialize PAI features
        pai_features = [0] * 5
        pai_options_displayed = False
        ...
        if show_pai_options == 'Yes':
            ...
            pai_options_displayed = True
            # st.write(pai_features)
    ...
    if st.button('Predict'):
        ...
        if show_pai_options == 'None':
            st.error('Please select an option for PAI Features.')

        elif model_periapical and model_pulpal:
